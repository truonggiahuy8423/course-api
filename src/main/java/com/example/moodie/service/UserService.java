...
import com.example.moodie.dto.request.LoginByEmailRequest;
import com.example.moodie.dto.request.LoginByPhoneRequest;
...
public class UserService {
    ...
    public LoginResponse loginByEmail(LoginByEmailRequest loginByEmailRequest) {
        ...
        if (!StringHandler.isNullOrEmptyOrBlank(loginByEmailRequest.getEmail())) {
            user = userRepository.findByEmail(loginByEmailRequest.getEmail()).orElseThrow(() ->
    ...
    public LoginResponse loginByPhone(LoginByPhoneRequest loginByPhoneRequest) {
        ...
        if (!StringHandler.isNullOrEmptyOrBlank(loginByPhoneRequest.getPhone())) {
            user = userRepository.findByEmail(loginByPhoneRequest.getPhone()).orElseThrow(() ->
                    new AppRuntimeException(ExceptionType.LOGIN_FAILED));
        }
...
//        return userRepository.findByUserId(userId).orElseThrow(() -> new AppRuntimeException(ExceptionType.NOT_FOUND));
